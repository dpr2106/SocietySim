  ...
  useEffect(() => {
    ...
    const tick = setInterval(() => {
      ...
      agents.forEach(agent => {
        ...
        // Phase 4: Factions
        if (resources.socialHarmony < 30) {
          if (['Anarchist', 'Villain', 'Hacker'].includes(agent.archetype)) {
            if (agent.faction !== 'Chaos') useSimStore.getState().updateAgent(agent.id, { faction: 'Chaos' });
          } else if (['Soldier', 'Politician', 'Elder'].includes(agent.archetype)) {
            if (agent.faction !== 'Order') useSimStore.getState().updateAgent(agent.id, { faction: 'Order' });
          }
        } else if (resources.socialHarmony > 60) {
          if (agent.faction && agent.faction !== 'Neutral') {
            useSimStore.getState().updateAgent(agent.id, { faction: 'Neutral' });
          }
        }
      });
    ...
    // Snapshot history every 5 seconds
    const snapshotTick = setInterval(() => {
      useSimStore.getState().snapshotHistory();
    }, 5000);

    // Agent-to-Agent Conversation Engine
    const convoTick = setInterval(async () => {
      const state = useSimStore.getState();
      if (isSimulating) return; // Don't chat during global crises

      const idleAgents = state.agents.filter(a => a.state === 'idle' || a.state === 'walking');
      if (idleAgents.length < 2) return;

      // Find a pair close to each other
      let pair: any = null;
      ...
      if (pair) {
        try {
          const res = await fetch('/api/converse', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ agent1: pair[0], agent2: pair[1] }),
          });
          const data = await res.json();
          if (data.success && data.dialogue) {
            data.dialogue.forEach((line: any) => {
              state.updateAgent(line.agentId, {
                state: 'socializing',
                speech: line.speech,
                speechVisible: true,
                targetPosition: pair[0].position, // Stop walking and face each other
              });
              
              // Automatically clear their speech bubble after 10 seconds
              setTimeout(() => {
                useSimStore.getState().clearSpeechBubble(line.agentId);
                useSimStore.getState().updateAgent(line.agentId, { state: 'idle' });
              }, 10000);
            });
          }
        } catch (e) {
    ...
    return () => {
      clearInterval(tick);
      clearInterval(snapshotTick);
      clearInterval(convoTick);
    };
  }, [isSimulating]);
